"""
Created on Fri  14 19:28:46 2022
...
from _legacy.exe_q20pes_taurus import TemplateDDInp, template_DD_input
from _legacy.exe_q20pes_taurus import _setDDTermInput, _printInput4Calculation, zipBUresults
import math as mth
...
def _set_deform_for_TES(res_0, b_lims, pair_args, 
                        keep_b20_limits=True, keep_def_limits=True):
    """ 
        Deform plane and PES starting form the energy minimum.
        ** MODIFICATION from exe_pairtes to include the opt. arg negative and
        behave as the b20_parameter
    ARGS:
    ...
    if p_base < p1: # left outter
        p1 = p_base - min(abs(p_base - p1), p_range / 2) # do not over extend the new range
        if keep_def_limits:
            p_range = p2 - p1
            Np = int(p_range / dp)
            Np = Np if Np % 2 == 1 else Np + 1   ## make an odd number of beta vals
        else:
            b2 = b_base + (b_range / 2)
        
    elif p_base > p2 : # right outter
        p2 = p_base + min(abs(p_base - p2), p_range / 2)
        if keep_def_limits:
            p_range = p2 - p1
            Np = int(p_range / dp)
            Np = Np if Np % 2 == 1 else Np + 1   ## make an odd number of beta vals
...
# blims      = (-0.25, 0.25, 5)
# DEF_CONSTR = Template.b32
# pargs   = (DEF_CONSTR, -1.5, 1.5, 10)

# res_0       = DataTaurus(12, 12, '', empty_data=True)
# res_0.b20_isoscalar = 0.3474
# setattr(res_0, DEF_CONSTR, 0.0)

# betas, pairs, defor_array = _set_deform_for_TES(res_0, blims, pargs, keep_b20_limits=1)
...
PRINT_DENS = """Spatial one-body density      {d_print}
Discretization for x/r        {r_print} 
Discretization for y/theta    {t_print}   0.00 
Discretization for z/phi      {p_print}   0.00 """


def _dens_printing_inpunt(print_dens=False):
    """ Export the density if print_dens=True """
    ## TODO: Check the EXPORTABLE PARAMETERS FOR EXPORT
    dr   = 0.075
    Rmax = 4
    NR   = int(Rmax // dr) + 1
    NTHE = 30
    ...
    kargs = dict(r_print='{:1}   {:4.2f}'.format(NR, dr),  
                 t_print=NTHE, p_print=NPHI)
    if not print_dens:
        txt = PRINT_DENS.format(d_print=0, **kargs)
    else:
        txt = PRINT_DENS.format(d_print=2, **kargs)
...
def _set_constrains(b_base, g_base, pkey, pval):
    ...
    for pk in (Template.b30, Template.b32):
        if pk == pkey:
...
def mainLinuxEvenBetaDeforms(z, n, interaction, b_lims=(-0.1, 0.1, 6), **kwargs):
    """ 
    ...
    pair_args    = ()
    pair_constr  = None
    DEF_VARS = (Template.b30, Template.b32)
    
    vals = kwargs['b_defs']
    if vals[0] in DEF_VARS:            
        pair_args   = vals #(k, vals[0], vals[1], vals[2])
        pair_constr = vals[0]
        #p_base      = min(vals[1:3])
    
    print("DEF_VARS", list(DEF_VARS))
    ...
    betas, pairs, deforms  = _set_deform_for_TES(*_args, keep_b20_limits=True,
                                                         keep_def_limits=True)
    ...
    kwargs[Template.grad_tol]   = 0.05
    kwargs[Template.eta_grad]   = 0.005
    kwargs[Template.mu_grad]    = 0.05
    key_order = []
    ## 3. COMPUTATION OF ALL DEFORMATIONS *************************************
    
    PRINT_HERE = [(0,p_lims[0]), (0,p_lims[1])]
    ...
    for i_b, i_p, b_def, p_def, cpwf, overwr_wf in deforms:
        ...
        ## print wf spatial density
        aux_ = _dens_printing_inpunt((i_b, i_p) in PRINT_HERE)
        kwargs[Template.dens_printing] = aux_
...
if __name__ == '__main__':
    ...
    if not os.getcwd().startswith('C:'):    #
        ...
        for z, n in nucleus:
            ...
            DEF_CONSTR = Template.b32 # .P_T10_J00 # 
            
            print("%%%   Constraining Pairng J,T: ", DEF_CONSTR)
            EXPORT_FILE = "export_B2vB3TESz{}n{}_{}_{}.txt"\
                                .format(z,n, tail, DEF_CONSTR.replace("_",""))
            DataTaurus.export_list_results = EXPORT_FILE
            print("%%%")
            
            # TODO: Fix the beta and the pairing exploration limits
            mainLinuxEvenBetaDeforms(z, n, interaction, 
                                     b_lims = (-0.60, 0.60, 11),
                                     b_defs = (DEF_CONSTR, -0.5, 0.5, 15),
                                     )
            
            zipBUresults(z, n, interaction, 'B20', DEF_CONSTR)
        ...
        if os.path.exists(import_file_Tau):
            ...
            with open(import_file_Tau, 'r') as f:
                ...
                for line in data:
                    ...
                    ## TODO: fix the index_export, (b, p) apear as p, b in export results
                    index_ = index_[1], index_[0]
            ...
            b_obl_prol = (B_dims[0], B_dims[1]+1, 1),
            # b_obl_prol = (B_dims[1], B_dims[0]-1,-1), # this plots the figure b20 reflected
            ...
            for bmin, bmax, stp in b_obl_prol:
                ...
                for i in range(bmin, bmax, stp):
                    ...
                    for j in range(*pa_lims):
                        ...
                        b20  = res.b20_isoscalar
                        b32  = getattr(res, DEF_CONSTR, -1)
                        
                        BP[index_] = (b20, b32)
                        IJ.append(index_)
                        
                        Bmax, Bmin = max(Bmax, b20), min(Bmin, b20)
                        Pmax, Pmin = max(Pmax, b32), min(Pmin, b32)
        ...
        cs2 = ax.contour(Bm, Pm, np.transpose(vals), 
                         colors=('w',), linewidths=(0.7,), levels=40)
        ...
        SPLITS  = 2 
        vlines_ = []
        for j in range(P_dims[0], P_dims[1]+1, SPLITS):
            x, y = [], []
            for i in range(B_dims[0], B_dims[1]+1):
                
                k = (j - P_dims[0]) \
                    + (i - B_dims[0])*(P_dims[1]-P_dims[0] + 1)
            ...
            vlines_.append(getattr(res, DEF_CONSTR))
                
            if len(x) > 0:
                ax[0].plot(x, y, '.-', label="{}={:5.3f}".format(DEF_CONSTR, P[j]))
        
        for i in range(B_dims[0], B_dims[1]+1, SPLITS):
            x, y = [], []
            for j in range(P_dims[0], P_dims[1]+1):
                ...
                x.append(getattr(res, DEF_CONSTR))
                # x.append(getattr(res, Template.P_T00_J10))
                y.append(getattr(res, attr_))
                
            if len(x) > 0:
                ax[1].plot(x, y, '.-', label="{}={:5.3f}".format("b20", B[i]))
        ax[0].legend()
        ax[1].legend()
        ax[0].set_ylabel(attr_)
        ax[0].set_xlabel('beta 20')
        ax[1].set_xlabel('beta 32')
        
        plt.title(attr_+"  {} {}  min={:6.3f}".format(z+n, nuclei[z], 
                                                     minVal), loc='center')
        for q in vlines_:
            ax[1].axvline(x=q, color='b', ls=':', lw=2,)
